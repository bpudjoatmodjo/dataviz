...
```{r}
gdp_id %>% 
   #ggplot(aes(x = year, y = total_gdp_including_oil_and_gas)) + geom_point()

   filter(year %in% c(1980,1985, 1990, 1995,2000)) %>% 
   ggplot(aes(year,total_gdp_including_oil_and_gas)) + 
...
gdp_id %>% 
  ...
  ggplot(aes(trade_hotel_and_restaurant,manufacturing , colour ="red")) +
  geom_point(alpha = 0.5) +
  scale_size_area()
```
...
gdp_id %>% 
  filter(province %in% c("Banten", "DKI Jakarta", "Jawa Barat", "Jawa Tengah", "Di Yogyakarta", "Jawa Timur")) %>% # ekstrak provinsi di Pulau Jawa
  ggplot(aes(x = trade_hotel_and_restaurant, y = manufacturing, colour = province, size =0.75)) +
  geom_point(alpha = 0.5)
```
...
```{r}
gdp_id %>% 
  filter(year %in% c("Banten", "DKI Jakarta", "Jawa Barat", "Jawa Tengah", "Di Yogyakarta", "Jawa Timur")) %>%
